...
function add(clearForm) {
    ...
    paymentForm
        .get('creditcards.newcreditcard.type')
        .setOptions(PaymentMgr.getPaymentMethod(PaymentInstrument.METHOD_CREDIT_CARD).activePaymentCards.iterator());
...
function save(params) {
    ...
    var card = {
        cardholderName: params.CreditCardFormFields.owner.value,
        number: params.CreditCardFormFields.number.value,
        expMonth: params.CreditCardFormFields.expiration.month.value,
        expYear: params.CreditCardFormFields.expiration.year.value,
        cvc: params.CreditCardFormFields.securityCode ? params.CreditCardFormFields.securityCode.value : ''
    };
    ...
    if (!customer.profile.custom.shift4payments__customerId) {
        var shift4Customer = CustomerHelpers.create(customer.profile, card, null);
        customer.profile.custom.shift4payments__customerId = shift4Customer.id;
        params.PaymentInstrument.setCreditCardToken(shift4Customer.cards[0].id);
    } else {
        var shift4Card = CustomerHelpers.addCard(customer.profile.custom.shift4payments__customerId, card, null);
        params.PaymentInstrument.setCreditCardToken(shift4Card.id);
    }
    /**
     * Shift4 Changes End
     * ------------------
...
function create() {
    ...
    try {
         ...
         */
        if (err.responseBody) {
            var responseBody = JSON.parse(err.responseBody);
            if (responseBody.error && responseBody.error.code) {
                var CardHelpers = require('*/cartridge/scripts/shift4/helpers/card');
                var status = CardHelpers.getErrorStatus(responseBody.error.code);
                var items = status.items.iterator();
                while (items.hasNext()) {
                    var item = items.next();
                    if (PaymentStatusCodes.CREDITCARD_INVALID_EXPIRATION_DATE === item.code) {
                        newCreditCardForm.get('expiration').get('month').invalidateFormElement();
                        newCreditCardForm.get('expiration').get('year').invalidateFormElement();
                    } else {
                        newCreditCardForm.get('number').invalidateFormElement();
                    }
                }
            }
        }
...
function Delete() {
    ...
    paymentForm.handleAction({
        remove: function (formGroup, action) {
            ...
            var CustomerHelpers = require('*/cartridge/scripts/shift4/helpers/customer');
            if (customer.profile.custom.shift4payments__customerId && paymentInstrument.creditCardToken) {
                CustomerHelpers.removeCard(
    ...
    if (verifyPaymentCardResult.error === true) {
        ...
        for (var i = 0; i < verifyPaymentCardResult.items.length; i++) {
            if (verifyPaymentCardResult.items[i].code === PaymentStatusCodes.CREDITCARD_INVALID_CARD_NUMBER) {
                ...
                verifyPaymentCardResult.items[i].code === PaymentStatusCodes.CREDITCARD_INVALID_EXPIRATION_DATE
            ) {
